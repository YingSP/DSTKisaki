local actions_status, actions_data = pcall(require, "kisaki_defs/action_defs")
    ...
    if actions_data.component_actions then
        for _, v in pairs(actions_data.component_actions) do
            ...
            local testfn = function(...)
                local rank = comp_data.type == "POINT" and -3 or -2
                local actions = GLOBAL.select(rank, ...)
                for _, data in pairs(comp_data.data) do
                    if data and data.checkfn and data.checkfn(...) then
                        data.action = string.upper(data.action)
                        table.insert(actions, GLOBAL.ACTIONS[data.action])
                    end
                end
            end
            AddComponentAction(comp_data.type, comp_data.component, testfn)
        end
    end
    --修改老动作
